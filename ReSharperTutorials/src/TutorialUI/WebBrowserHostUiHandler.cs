...
{
    /// <summary>
    /// Used to force WebBrowser to support high DPI and disable context menu
    /// </summary>
    public class WebBrowserHostUiHandler : Native.IDocHostUIHandler
    {
        ...
        private bool _isOneTimeRefreshed = false;
        ...
        public WebBrowserHostUiHandler(WebBrowser browser)
        {
            ...
            Browser = browser;
            Browser.DocumentCompleted += OnLoadCompleted;
        ...
        private void OnLoadCompleted(object sender, WebBrowserDocumentCompletedEventArgs e)
        {
            var doc = Browser?.Document?.DomDocument as Native.ICustomDoc;
            doc?.SetUIHandler(this);

            // force running of GetHostInfo to propagate DPI_AWARE flag
            if (!_isOneTimeRefreshed && Browser != null)
            {
                Browser.Refresh(WebBrowserRefreshOption.Completely);
                _isOneTimeRefreshed = true;
            }
        }
        ...
        {
            if (IsWebBrowserContextMenuEnabled) return S_FALSE;
            else return S_OK;
        }
        ...
        uint Native.IDocHostUIHandler.TranslateAccelerator(ref System.Windows.Forms.Message msg, ref Guid group,
            int nCmdID)
        {
            return S_FALSE;
        }
        ...
        {
            var sp = browser?.Document?.DomDocument as Native.IOleServiceProvider;
            if (sp == null) return;
